...
export default class ConnectionDialog extends Component {
  ...
  constructor(props: PropsType) {
    ...
    if (lastState) {
      ...
      this.state = {
        ...
        mountDirectory: props.mountDirectory
      };
    }
  }
        ...
        <form onSubmit={this._onSubmit}>
          <div className="connection-dialog">
            ...
            <div className="connection-dialog-row">
              ...
              <div className="connection-dialog-cell">
                <input
                  value={this.state.serverCommand}
                  onChange={e => this.setState({serverCommand: e.target.value})}
                />
              </div>
            </div>
            <div className="connection-dialog-row">
              ...
              <div className="connection-dialog-cell">
                <input
                  value={this.state.searchDirectory}
                  onChange={e =>
                    this.setState({searchDirectory: e.target.value})}
                />
              </div>
            </div>
            <div className="connection-dialog-row">
              <div className="connection-dialog-cell">Local (mount) Directory:</div>
              <div className="connection-dialog-cell">
                <input
                  value={this.state.mountDirectory}
                  onChange={e =>
                    this.setState({mountDirectory: e.target.value})}
                />
              </div>
            </div>
          </div>
          <div className="connection-dialog-submit-button-container">
            <button className="connection-dialog-submit-button" type="submit">
              CONNECT
            </button>
          </div>
        </form>
  ...
  _onSubmit(event: Event) {
    ...
    this.props.onConnect(
      ...
      this.state.serverCommand,
      this.state.searchDirectory,
      this.state.mountDirectory
    );
